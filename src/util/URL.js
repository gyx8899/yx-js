 ...
 */
import {call} from './Tool';

function getFileNameFromURL(url) {
	const fileNameSplit = url.split('/').pop().split('#')[0].split('?')[0].split('.');
	return {
...
let getURLParamByName = (name, url = window.location.search) => {
	if (URLSearchParams) {
...
function removedLocationUrlParam(param) {
	...
	const removedParamUrl = removeUrlParam(locationUrl, param);
